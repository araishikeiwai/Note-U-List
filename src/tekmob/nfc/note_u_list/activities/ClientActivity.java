...
public class ClientActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		wifiClientReceiver = new WiFiClientBroadcastReceiver(wifiManager,
				wifichannel, this);
	...
	public void setNetworkToReadyState(boolean status, WifiP2pInfo info,
		...
		if (resultCode == Activity.RESULT_OK && requestCode == fileRequestID) {
			...
			if (targetDir.isFile()) {
				if (targetDir.canRead()) {
					...
					setTargetFileStatus(targetDir.getName()
	...
	public void sendFile(View view) {
		...
		if (!transferActive) {
			...
			/*
			 * else if(targetDevice == null) { setClientFileTransferStatus(
			 * "Target Device network information unknown"); }
			 */
				...
				clientServiceIntent.putExtra("clientResult",
						new ResultReceiver(null) {
							...
							protected void onReceiveResult(int resultCode,
								...
								if (resultCode == port) {
									if (resultData == null) {
										// Client service has shut down, the
										// transfer may or may not have been
										// successful. Refer to message
										transferActive = false;
									} else {
	...
	protected void onPause() {
		super.onPause();
		// Continue to listen for wifi related system broadcasts even when
		// paused
		// stopClientReceiver();
	}
