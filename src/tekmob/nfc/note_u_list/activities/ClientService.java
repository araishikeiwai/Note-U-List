...
public class ClientService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		// targetDevice = (WifiP2pDevice)
		// intent.getExtras().get("targetDevice");
		wifiInfo = (WifiP2pInfo) intent.getExtras().get("wifiInfo");

		if (!wifiInfo.isGroupOwner) {
			// targetDevice.
			// signalActivity(wifiInfo.isGroupOwner + " Transfering file " +
			// fileToSend.getName() + " to " +
			...
			try {
				 ...
				 * 
				 * if(!inputData.equals("wdft_server_hello")) { throw new
				 * IOException("Invalid WDFT protocol message");
				 * 
				 * }
				 * 
				 * 
				 * pw.println(fileToSend.getName());
				 * 
				 * if(!inputData.equals("wdft_server_ready")) { throw new
				 * IOException("Invalid WDFT protocol message");
				 * 
				 * }
				 */
